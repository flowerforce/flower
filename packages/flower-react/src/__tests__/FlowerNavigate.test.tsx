...
const Text = ({ text, value }: {text?: string, value?: any, id?: any, children?: any}) => {
  return <h1 data-testid="h1">{text || value}</h1>
}

const ButtonNode = ({ route }) => {
  return (
...
const ButtonNextFunc = ({ route, dataIn }: any) => {
  return (
    <FlowerNavigate action="onNext" route={dataIn || route}>
      {({ onClick }) => <button data-testid="btn-next" onClick={onClick}>NEXT</button>}
    </FlowerNavigate>
  )
}
    ...
    render(
      <FlowerProvider>
        <Flower name="app-test">
          ...
          <FlowerNode id="form">
            <FlowerNavigate action="onNext" rules={{ amount: { $eq: 2 } }} alwaysDisplay>
    ...
    render(
      <FlowerProvider>
        <Flower name="app-test5">
          <FlowerRoute onEnter={()=>{
            console.log('start')
          }} id="start" to={{ step1: null }} />
          <FlowerNode id="step1" to={{ step2: null }}>
            <Text text="step1"></Text>
            <FlowerNavigate action="onNext">
              {({ onClick, hidden }) => (
                <button data-testid="btn-next" onClick={onClick} disabled={hidden}>NEXT</button>
              )}
            </FlowerNavigate>
          </FlowerNode>
