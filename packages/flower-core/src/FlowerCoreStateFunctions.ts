...
export const FlowerCoreReducers: ReducersFunctions = {
  ...
  historyPrevToNode: (state, { payload }) => {
    const history = _get(state[payload.name], ["history"], []);
    const lastIndex = lastIndexOf(history, payload.node);
    // se passo un nodo che non esiste
    if (lastIndex >= 0) {
      const newHistory = _slice(history, 0, lastIndex + 1);
      _set(state, [payload.name, "history"], newHistory);
      _set(state, [payload.name, "current"], payload.node);
    }
    return state;
  },
  ...
  setFormTouched: (state, { payload }) => {
    _set(
      state,
      [payload.flowName, "form", payload.currentNode, "touched"],
      true
    );
    return state;
  },
  ...
  node: (state, { payload }) => {
    ...
    console.log(_get(global.window, "__FLOWER_DEVTOOLS__"), 'dev tools key **************** --------')
    console.log(history, 'history  **************** --------')
